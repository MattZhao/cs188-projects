...
def numpy_array_from_counters(counters):
    assert isinstance(counters, list)
    return np.array([[v for (k, v) in sorted(datum.items())] for datum in counters])

def counters_from_numpy_array(array):
    assert array.ndim == 2
    counters = [None] * array.shape[0]
    counter_keys = range(0, array.shape[1])

    for i in range(len(array)):
        counters[i] = Counter()
        counters[i].update(zip(counter_keys, array[i]))
    return counters

def list_from_numpy_array_one_hot(array):
    assert array.ndim == 2
    # assert that array is a one-hot representation
    assert np.all((array == 1).sum(axis=1) == 1)
    assert np.all((array == 0).sum(axis=1) == (array.shape[1] - 1))
    labels = np.argmax(array, axis=-1).tolist()
    return labels
